...
export default function Streams({
  ...
  useEffect(() => {
    ...
    const fetchStreams = async () => {
      ...
      try {
        ...
        setQueue(
          streams?.sort(
            (a: QueueItem, b: QueueItem) => b?.upvotes - a?.upvotes,
          ),
        );
        if (currentStream) {
  ...
  const handlePlayStream = async (auto: boolean) => {
    console.log(
      "handlePlayStream start",
    ...
    console.log(
     ...
     console.log("handlePlayStream called", { auto, queueLength: queue.length, currentSong: currentSong?.extractedId });
 

    try {
      if (queue.length === 0) {
        toast.message("No Next Song found!");
        if (auto) {
          setQueue([]);
          setCurrentSong(null);
        }
          ...
          <section className="flex flex-col gap-6 lg:col-span-7">
            <div className="flex items-center justify-between">
              ...
              <button
                ...
                className=" text-zinc-500 border py-2 px-4 rounded-xl  hover:bg-fuchsia-300/90 active:bg-fuchsia-300 cursor-pointer "
              >
                Share
              </button>
            </div>
              ...
              <div className="relative aspect-video">
                ...
                title="YouTube video player"
                 ></iframe> */}
                    {isAdmin ? (
                      // <YoutubeVideoPlayer
                      //  key={currentSong.extractedId} 
                      //   videoId={currentSong.extractedId || ""}
                      //   onEnd={() => handlePlayStream(true)}
                      // />
                      <YoutubeVideoPlayer
  key={currentSong.extractedId}
  videoId={currentSong.extractedId || ""}
  onEnd={() => {
    console.log("onEnd fired, currentSong:", currentSong?.extractedId);
    handlePlayStream(true);
  }}
/>
                    ) : (
                      <Image
            ...
            {isAdmin && (
              <button
                ...
                onClick={() => handlePlayStream(false)}
            ...
            <div className="border p-6 mt-2 rounded-xl">
              ...
              <div className="grid gap-4 md:grid-cols-2">
                {queue?.length === 0 ? (
                  <span className="text-center">No songs found</span>
                ) : (
                  queue.map((item) => (
